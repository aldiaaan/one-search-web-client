...
const __cache = {} as Record<string, (typeof countries)[0]>

export function getCountryDetailsByCode(code?: string) {
  if (!code) return
  if (__cache[code]) return __cache[code]

  const country = countries.find((c) => c.code === code)
  
  if (country) {
    __cache[code] = country
  }
