...
import { OneNotFoundState } from '@/components/OneState'
...
function getTableRowClasses(options: { disableGutter?: boolean }) {
    ...
    <thead class="border-b border-g-100">
      <tr class="text-xs font-medium">
        <th
          ...
          :key="header.key"
          class="py-3 px-6 text-start text-gray-500"
        >
          {{ header.title }}
        </th>
      </tr>
    ...
    <tbody v-if="!isLoading">
      <tr v-for="(item, index) in items" :key="index" class="border-b border-gray-100">
        <td
          ...
          :width="headers?.find((header) => header.key === key)?.width"
          :key="key"
          v-for="key in headers?.map((header) => header.key)"
          :class="
            getTableRowClasses({
              disableGutter: Boolean(
                headers?.find((header) => header.disableGutter && header.key === key)
              )
            })
          "
        >
