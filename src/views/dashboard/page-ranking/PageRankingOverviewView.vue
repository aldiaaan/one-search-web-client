...
<template>
  <OneDialog :open="shouldOpenStartDocumentRankingDialog" title="Start New Service"
    ...
    <template #footer>
      <OneButton class="w-20" @click="() => {
        start({
          iterations,
          dampingFactor
        });
        shouldOpenStartDocumentRankingDialog = false
      }" :is-loading="isLoadingStart">Start</OneButton>
    </template>
  </OneDialog>
  <div class="container mx-auto px-8 flex flex-col gap-6">
    ...
    <div class="grid grid-cols-3 gap-6">
      <div class="col-span-2 flex flex-col gap-6">
        ...
        <div class="border border-gray-200 rounded-lg  bg-white">
          ...
          <div class="flex flex-col flex-1 gap-4">
            <OneTable :is-loading="isLoadingTopWebpages" :headers="headers"
      ...
      <div class="flex flex-col gap-6">
        <OneMetrics title="Disk Usage" :value="formatFileSize(metrics?.fileSize)" :is-loading="isLoadingMetrics">
        </OneMetrics>
        <OneMetrics title="Total Linking" :value="formatNumber(metrics?.totalLinking)" :is-loading="isLoadingMetrics">
        </OneMetrics>
        <OneMetrics title="Total Nodes" :value="formatNumber(metrics?.totalNodes)" :is-loading="isLoadingMetrics">
        </OneMetrics>
        <OneMetrics title="Page Rank Vector Size" :value="metrics?.vectorSize" :is-loading="isLoadingMetrics">
        </OneMetrics>
      </div>
    </div>
  </div>
